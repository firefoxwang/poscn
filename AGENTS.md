...
- **Commits**: Do not add `Co-authored-by:`, `Signed-off-by:`, or any Cursor/agent/IDE attribution to commit messages. Do not advertise the agent or tool in commits. To enforce this locally, run `./scripts/install-git-hooks.sh` once (installs a prepare-commit-msg hook that strips such lines). **Always commit completed work**: when you finish a change, feature, or fix that the user asked for, stage and commit the changes so they are not lost. Do not leave the user to ask "was this committed?" — commit as part of doing the needed. **Use SSH for git remote**: this repo (fork `firefoxwang/poscn`) uses `id_rsa`; keep origin as **`ssh://git@ssh.github.com:443/firefoxwang/poscn.git`** (SSH over GitHub's port-443 endpoint, NOT HTTPS and NOT `git@github.com:…`) so fetch/push work without prompting. The standard `git@github.com:22` is blocked by the local proxy (connections close at `198.18.0.218`); the `ssh.github.com:443` URL carries its own host/port, so **no `~/.ssh/config` entry is needed** — `id_rsa` authenticates as `firefoxwang` (verified with `ssh -T git@ssh.github.com -p 443`). If the remote ever reverts to HTTPS or `git@github.com`, restore this exact URL before any fetch/push.
...
**本项目定位：** 本项目是基于原 POS（satisfecho/pos）的二次开发项目，主要用于中国国内本地化改造。涉及支付系统（如支付宝、微信支付）、个税（如个人所得税计算/个税合规相关逻辑）、地图（如地图服务切换为国内可用的高德/腾讯地图等）等模块，均需按中国本地法规、服务和环境进行本地化调整。做任何相关改动时，请优先考虑中国本地化的约束与合规要求。
