...
| `POST /mp/auth/bind-phone` | Bearer **customer** token | Consumer binds the WeChat phone to the logged-in customer: `{code, nickname?}`. Resolves `get_phone_number` using the customer's own binding appid/secret, stores the phone (E.164, prefers WeChat `countryCode`) on `Customer.phone` and `MpBinding.phone`, updates nickname when provided → `{profile, phone}`. `404 no_binding` if the customer has no MP binding. |
...
1b. **Bind phone (optional first-login step):** if the login response `profile.phone` is empty, the login page shows a 「获取微信手机号」 step (`<button open-type="getPhoneNumber">` → `e.detail.code`) → `POST /mp/auth/bind-phone`, which stores the phone on `Customer`/`MpBinding`. The user may 「暂不绑定，直接进入」 to skip; the phone stays empty until bound.
...
- `test_mp_auth_flows.py` — login (customer auto-create + reuse, merchant 403 unbound), bind-staff (happy path, already-bound 409, wrong password 401, OTP required / wrong OTP), phone, bind-phone (happy path + anonymous 401), refresh, me (staff + customer + anonymous 401).
